...
def get_tank_details(email: str, password: str) -> dict:
    ...
    # Initialize tank details dictionary
    tank_details = {}
    ...
    if oil_estimate:
        try:
            # Extract the number from text like "We estimate that you have 530 litres of usable oil left in your tank"
    ...
    # Get tank model
    tank_model_input = soup.find("input", {"id": "tank_model"})
    if tank_model_input and tank_model_input.get("value"):
        tank_details["model"] = tank_model_input["value"]
        print("Found tank model")
