...
if usable_level_div:
    ...
    if oil_level and oil_level.get("data-percentage"):
        ...
        if "total_level_percentage" not in data:
            ...
            print(
                f"✓ Using oil level as total level (new interface): {level_percentage}%"
            )

# Look for volume information in text
volume_texts = soup.find_all(
    string=lambda text: (
        text and any(word in text.lower() for word in ["litre", "volume", "oil level"])
    )
)
...
if "current_volume_litres" in data and "usable_volume_litres" not in data:
    ...
    print(
        f"✓ Using current volume as usable volume: {data['usable_volume_litres']} litres"
    )
