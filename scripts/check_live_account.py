...
def test_connection(session: requests.Session, email: str, password: str) -> bool:
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"Error: Failed to access login page with status code {response.status_code}"
            )
        ...
        if not csrf_token:
            ...
            for meta in soup.find_all("meta"):
                print(
                    f"- {meta.get('name', 'no name')}: {meta.get('content', 'no content')}"
                )
            return False
        ...
        if response.status_code != 200:
            print(
                f"Error: Failed to access account page with status code {response.status_code}"
            )
            return False
        ...
        print(f"\nError during connection test: {e!s}")
        return False
...
def get_tank_id(session: requests.Session) -> str | None:
    ...
    if response.status_code != 200:
        print(
            f"Error: Failed to access tanks page with status code {response.status_code}"
        )
        print(f"Response URL: {response.url}")
        return None
...
def get_tank_details(session: requests.Session, tank_id: str) -> dict | None:
    ...
    if usable_level_div:
        ...
        if oil_level and oil_level.get("data-percentage"):
            tank_details["usable_level_percentage"] = float(
    ...
    price_texts = soup.find_all(
        string=lambda text: (
            text
            and any(
                word in text.lower()
                for word in ["pence", "p/litre", "£", "cost", "price"]
            )
        )
    )
                        ...
                        if bracket_count == 0:
                            try:
                                json_data = json.loads(
                                    script.string[array_start:array_end]
                                )
                                for item in json_data:
                                    if str(item.get("id")) == str(model_id):
                                        print("\nFound tank data in JSON:")
                                        print(json.dumps(item, indent=2))
                                        tank_details["model"] = item.get(
                                            "tank", {}
                                        ).get("Description")
                                        tank_details["manufacturer"] = item.get(
                                            "tank", {}
                                        ).get("Brand")
    ...
    try:
        ...
        for element in price_elements:
            ...
            if price_match is not None:
                return {"price_pence": price_match, "source_text": text}

        print("Could not find current price on the kerosene prices page")
        return None

    except Exception as e:
        print(f"Error getting oil price: {e!s}")
        return None
...
def main():
    ...
    if not email or not password:
        print(
            "Error: Please set BOILERJUICE_EMAIL and BOILERJUICE_PASSWORD environment variables"
        )
        sys.exit(1)
